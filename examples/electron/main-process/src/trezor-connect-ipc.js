...
const { TRANSPORT_EVENT, UI, UI_EVENT, DEVICE_EVENT, TRANSPORT, DEVICE } = require('@onekeyhq/connect');

let inited = false;
// SETUP trezor-connect
exports.initOneKeyConnect = (sender) => {
    ...
    OneKeyConnect.init({
        ...
        sender.send('trezor-connect', 'OneKeyConnect is ready!');
    }).catch(error => {
        sender.send('trezor-connect', 'OneKeyConnect init error:' + error);
    });
};

exports.callOneKeyConnect = (sender, message) => {
    const { method, params } = message;
    OneKeyConnect[method](params).then(response => {
